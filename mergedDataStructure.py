...
class MergedDataStructure():
    ...
    def __init__(self, delta=4, filename="sp500Week.csv"):
        ...
        for i in range(0,limit-1):
            ...
            dateList = [datetime.datetime.strptime(Date[i+1], "%Y-%m-%d") - datetime.timedelta(days=x) for x in range(0, ( datetime.datetime.strptime(Date[i+1], "%Y-%m-%d")- datetime.datetime.strptime(Date[i], "%Y-%m-%d") ).days )]
